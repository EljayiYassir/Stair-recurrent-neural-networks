...
class RNNModel(nn.Module):
    ...
    def forward(self, input,hidden):
        ...
        for i in range(emb.shape[0]):
            if NVAR != 'none':
                hidden = getdecay(i) * self.rnn(emb[i], hidden)
            else:
...
parser.add_argument('--nvar',
                    ...
                    choices=['none','1n', '1n2', 'expn', 'logn', 'log2n', 'sqrtlogn'],
...
for rnd in range(rounds):
    ...
    print('=' * 89)
    print('| End of training | test loss {:5.2f} | test ppl {:8.2f}, test bpc {:8.2f} | test acc {:8.2f} | Average time per epoch in seconds : {}|'.format(
        test_loss, math.exp(test_loss), test_loss / math.log(2), test_accuracy, sum(time_per_epochs) / float(len(time_per_epochs))))
    print('=' * 89)
