...
class Model(nn.Module):
    def __init__(self, hidden_size, rec_net):
        ...
        self.alpha = nn.Parameter(torch.ones(SLEN))
    ...
    def forward(self, x, y):
        ...
        for i in range(len(x)):
            if args.onehot:
                inp = onehot(x[i])
                hidden = self.alpha[i] * self.rnn.forward(inp, hidden)
            else:
                hidden = self.alpha[i] * self.rnn.forward(x[i], hidden)
            out = self.lin(hidden)
...
def train_model(net, optimizer, batch_size, T, n_steps):
    ...
    with open(SAVEDIR + '{}_Train_Alpha'.format(NET_TYPE),'wb') as fp:
        pickle.dump(alphas, fp)
        
    with open(SAVEDIR + '{}_Train_NORM'.format(NET_TYPE), 'wb') as fp:
        pickle.dump(norm,fp)

    with open(SAVEDIR + '{}_TestDat.txt'.format(NET_TYPE), 'w') as fp:
        fp.write(f"train accuracy: {max(accs)} \t train loss: {min(losses)}")
...
SAVEDIR = os.path.join('./saves', 'copytask',
                       NET_TYPE, args.name, str(random_seed),exp_time)
